...
if (isset($_POST['deleteSelected'])) {
    ...
    if (!empty($deleteIds)) {
        ...
        $deleteQuery = "DELETE FROM custlist WHERE id IN ($idString)";
        $deleteResult = $conn->query($deleteQuery);

        if ($deleteResult) {
            echo "<script>alert('Successful')</script>";
            echo "<script>setTimeout(function(){ window.location.href = 'adcust.php'; }, 500);</script>";
        } else {
...
<body>
    ...
    <script>
        function createInvoice() {
            ...
            var checkedIds = Array.from(checkboxes).filter(checkbox => checkbox.checked).map(checkbox => checkbox.value);

            if (checkedIds.length === 1) {
                var customerId = checkedIds[0];
                window.location.href = 'invoice.php?customerId=' + customerId;
            } else {
                alert('Please select exactly one customer to create an invoice.');
            }
        }
    </script>
